			...
			describe("activity", function () {
				...
				*/
		        ...
		        it("should throw Error when update is called - Update is not allowed for activity", function () {
		            var activity = createobject( "component", "whispirColdFusionSDK.resources.activity" ).init(whispirClientMock);
		            expect(function () { activity.update(); }).toThrow(regex='Updating a activity is not supported via the API');
		        ...
		        it("should throw Error when delete is called - Delete is not allowed for activity", function () {
		            var activity
		             = createobject( "component", "whispirColdFusionSDK.resources.activity" ).init(whispirClientMock);
		            expect(function () { activity.delete(); }).toThrow(regex='Deleting a activity is not supported via the API');
